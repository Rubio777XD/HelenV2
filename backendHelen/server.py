...
def _decode_fourcc(raw: float) -> str:
    try:
        code = int(raw)
    except (TypeError, ValueError):  # pragma: no cover - depende de OpenCV
        return ""
...
def _detect_pi_model() -> str:
    ...
    if not model_path.exists():
        return ""
    with contextlib.suppress(Exception):
        return model_path.read_text(encoding="utf-8", errors="ignore").strip("\0\n")
    return ""
...
class RuntimeConfig:
    ...
    camera_index: int = 0
    detection_confidence: float = 0.88
    tracking_confidence: float = 0.86
...
class HealthSnapshot:
    ...
    processing_resolution: Optional[Tuple[int, int]]
    processing_fps: Optional[float]
    presentation_resolution: Optional[Tuple[int, int]]
    presentation_fps: Optional[float]
    latency_stats: Dict[str, float]
    dataset_path: str
    dataset_uses_fallback: bool
...
class CameraGestureStream:
    ...
    def __init__(
        ...
        *,
        ...
        normalized_hint: Dict[str, Optional[Any]] = (
            camera_probe.normalize_camera_target(camera_identifier)
            if camera_identifier
            else {"identifier": None, "index": None, "path": None}
        )
        explicit_index = normalized_hint.get("index")
        explicit_path = normalized_hint.get("path")

        if selection and selection.index is not None:
            camera_index = selection.index
        elif explicit_index is not None:
            camera_index = int(explicit_index)

        self._camera_identifier = normalized_hint.get("identifier") if camera_identifier else None
        self._explicit_path = explicit_path
        self._camera_index = camera_index
        self._detection_confidence = detection_confidence
        self._tracking_confidence = tracking_confidence
        self._metrics = metrics
        self._device_path = selection.device if selection and selection.device else explicit_path
        ...
        self._pixel_format: Optional[str] = selection.pixel_format if selection else None
        ...
        if selection and selection.width and selection.height:
            self._capture_resolution: Tuple[int, int] = (
                int(selection.width),
                int(selection.height),
            )
        else:
            self._capture_resolution = (0, 0)
        self._capture_fps = float(selection.fps) if selection and selection.fps else 0.0

        if profile:
            self._presentation_resolution: Tuple[int, int] = (int(profile.width), int(profile.height))
            self._presentation_fps = float(profile.fps)
        elif selection and selection.width and selection.height:
            self._presentation_resolution = (
                int(selection.width),
                int(selection.height),
            )
            self._presentation_fps = float(selection.fps) if selection and selection.fps else 0.0
        else:
            self._presentation_resolution = (0, 0)
            self._presentation_fps = 0.0

        self._processing_resolution: Optional[Tuple[int, int]] = None
        self._processing_fps = 0.0
        self._previous_capture_timestamp: Optional[float] = None
        ...
        kind_hint = selection.kind if selection else None
        if not kind_hint and (self._device_path or "").startswith("/dev/video"):
            kind_hint = "usb"
        self._is_uvc = kind_hint == "usb"
    ...
    def _configure_capture(self, cap: Any) -> None:
        if self._is_uvc and hasattr(cv2, "VideoWriter_fourcc"):
            with contextlib.suppress(Exception):
    ...
    # ------------------------------------------------------------------
    def _snapshot_capture_metrics(self, cap: Any) -> None:
        ...
        with contextlib.suppress(Exception):
            decoded = _decode_fourcc(cap.get(cv2.CAP_PROP_FOURCC))
            if decoded:
                self._pixel_format = decoded
    ...
    def _attempt_v4l2(self) -> Tuple[Optional[Any], Optional[str]]:
        ...
        self._snapshot_capture_metrics(cap)
        actual_width, actual_height = self._capture_resolution
        actual_fps = self._capture_fps
        pixel_format = self._pixel_format or "?"
    ...
    def _attempt_gstreamer(self) -> Tuple[Optional[Any], Optional[str]]:
        ...
        actual_fps = self._capture_fps
        LOGGER.info(
            "Ruta de cámara inicializada: gstreamer (pipeline=%s, %sx%s @ %.2f fps)",
            pipeline,
            actual_width,
            actual_height,
            actual_fps,
    ...
    def _should_attempt_reconnect(self) -> bool:
        ...
        self._reconnect_backoff = min(self._reconnect_backoff * 1.5, 8.0)
        return True
    ...
    def _reopen_capture(self) -> bool:
        try:
            ...
            LOGGER.warning("Intento de reconexión fallido: %s", error)
    ...
    def close(self) -> None:
        ...
        self._processing_resolution = None
        self._processing_fps = 0.0
        ...
        while True:
            ...
            if not ok or frame is None:
                ...
                if self._capture_backend == "v4l2" and self._switch_to_gstreamer():
                    ...
                    time.sleep(0.1)
                    continue
                if self._should_attempt_reconnect() and self._reopen_capture():
                    LOGGER.warning(
                        "Reintentando apertura de cámara tras %d fallos",
            ...
            self._consecutive_failures = 0
            self._reconnect_backoff = 0.5
            self._snapshot_capture_metrics(self._cap)
            ...
            self._processing_resolution = (int(width), int(height))
            ...
            now = time.time()
            if self._previous_capture_timestamp:
                delta = now - self._previous_capture_timestamp
                if delta > 0:
                    self._processing_fps = 1.0 / delta
            self._previous_capture_timestamp = now
            self._last_capture = now
            if (not self._presentation_resolution) or (
                isinstance(self._presentation_resolution, tuple)
                and len(self._presentation_resolution) >= 2
                and (self._presentation_resolution[0] <= 0 or self._presentation_resolution[1] <= 0)
            ):
                self._presentation_resolution = (int(width), int(height))
            if self._presentation_fps <= 0.0 and self._processing_fps > 0.0:
                self._presentation_fps = float(self._processing_fps)
            self._last_error = None
            self._healthy = True
    ...
    def status(self) -> Dict[str, Any]:
        status: Dict[str, Any] = {
            ...
            "presentation_resolution": self._presentation_resolution,
            "presentation_fps": self._presentation_fps,
            "last_capture": self._last_capture,
            "last_error": self._last_error,
...
class HelenRuntime:
    ...
    def __init__(self, config: Optional[RuntimeConfig] = None) -> None:
        ...
        pi_model = _detect_pi_model() or "(modelo no detectado)"
        LOGGER.info(
            "Arrancando HELEN en %s (Python %s)",
            pi_model,
            platform.python_version(),
        )
        LOGGER.info(
            "Parámetros de MediaPipe: detection=%.2f tracking=%.2f poll_interval=%.3f s frame_stride=%d",
            float(self.config.detection_confidence),
            float(self.config.tracking_confidence),
            float(self.config.poll_interval_s),
            int(self.config.process_every_n),
        )
        ...
        LOGGER.info(
            "Dataset activo: %s (fallback=%s, existe=%s)",
            dataset_path,
            using_fallback,
            dataset_path.exists(),
        )
        if self._camera_selection:
            ...
            LOGGER.info(
                "Selección de cámara: backend=%s dispositivo=%s (%sx%s @ %.2f fps, formato=%s, %.1f ms)",
                selection.backend,
                selection.device or selection.index,
                selection.width,
                selection.height,
                selection.fps,
                selection.pixel_format or "<desconocido>",
    ...
    def _ensure_camera_selection(self, *, force: bool) -> Optional[CameraSelection]:
        preferred_hint: Optional[Union[str, int]] = None
        identifier = getattr(self.config, "camera_identifier", None)
        if identifier:
            normalized = camera_probe.normalize_camera_target(identifier)
            preferred_hint = normalized.get("path") or normalized.get("index")
        if preferred_hint is None:
            camera_index = getattr(self.config, "camera_index", None)
            if isinstance(camera_index, int):
                preferred_hint = camera_index

        try:
            selection = camera_probe.ensure_camera_selection(
                force=force,
                preferred=preferred_hint,
                logger=LOGGER,
            )
        ...
        if selection:
            ...
            device_hint = selection.device or (f"/dev/video{selection.index}" if selection.index is not None else None)
            if device_hint:
                self.config.camera_identifier = device_hint
        return selection
        ...
        if self.config.enable_camera:
            ...
            try:
                stream = CameraGestureStream(
                    ...
                    camera_identifier=self.config.camera_identifier,
    ...
    def health(self) -> HealthSnapshot:
        ...
        def _tuple_from(value: Any) -> Optional[Tuple[int, int]]:
            if isinstance(value, (tuple, list)) and len(value) >= 2:
                return int(value[0]), int(value[1])
            return None

        capture_resolution = _tuple_from(stream_status.get("capture_resolution"))
        processing_resolution = _tuple_from(stream_status.get("processing_resolution"))
        if not processing_resolution and stream_status.get("frame_shape"):
            frame_shape = stream_status.get("frame_shape")
            if isinstance(frame_shape, (tuple, list)) and len(frame_shape) >= 2:
                processing_resolution = int(frame_shape[1]), int(frame_shape[0])
        presentation_resolution = _tuple_from(stream_status.get("presentation_resolution"))

        camera_last_capture_raw = stream_status.get("last_capture")
        camera_last_capture = (
            _iso_timestamp(camera_last_capture_raw)
            if camera_last_capture_raw
            else None
        )
        ...
        return HealthSnapshot(
            ...
            camera_device=stream_status.get("device_path") or stream_status.get("camera_identifier"),
            camera_last_capture=camera_last_capture,
            camera_last_error=stream_status.get("last_error"),
            camera_pixel_format=stream_status.get("pixel_format"),
            capture_resolution=capture_resolution,
            capture_fps=float(stream_status.get("capture_fps") or 0.0) if capture_resolution else None,
            processing_resolution=processing_resolution,
            processing_fps=float(stream_status.get("processing_fps") or 0.0)
            if processing_resolution
            else None,
            presentation_resolution=presentation_resolution,
            presentation_fps=float(stream_status.get("presentation_fps") or 0.0)
            if presentation_resolution
            else None,
            latency_stats=latency_stats,
            dataset_path=str(self.config.dataset_path),
            dataset_uses_fallback=bool(self.dataset_info.get("using_fallback")),
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--camera-index",
        default="0",
        help="Cámara a utilizar (índice numérico o ruta /dev/videoX)",
    )
    ...
    normalized_camera = camera_probe.normalize_camera_target(args.camera_index)
    camera_identifier = normalized_camera.get("path") or normalized_camera.get("identifier")
    camera_index_value: Optional[int] = normalized_camera.get("index")
    if camera_index_value is None:
        try:
            camera_index_value = int(str(args.camera_index).strip())
        except (TypeError, ValueError):
            camera_index_value = 0

    config = RuntimeConfig(
        camera_identifier=camera_identifier,
        camera_index=int(camera_index_value or 0),
